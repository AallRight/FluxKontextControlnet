...
class FluxControlNetInpaintingPipeline(DiffusionPipeline, FluxLoraLoaderMixin):
    ...
    #             batch_size, image_latent_height, image_latent_width, device, dtype
    #         )
    #         # image ids are the same as latent ids with the first dimension set to 1 instead of 0
    #         image_ids[..., 0] = 1
        
    #     latent_ids = self._prepare_latent_image_ids(batch_size, height, width, device, dtype)

    #     if latents is not None:
    #         latent_image_ids = self._prepare_latent_image_ids(
    #             batch_size, height, width, device, dtype
    #         )
    #         return latents.to(device=device, dtype=dtype), image_latents, latent_ids, image_ids
    ...
    def __call__(
        self,
        kontext_image: PipelineImageInput | None = None,
        prompt: Union[str, List[str]] = None,
        ...
        # ? added
        self._current_timestep = None # ? added
        # ? added
        ...
        # 3.0. Preprocess image
        # ? 这是准备kontext图像
        if kontext_image is not None and not (isinstance(kontext_image, torch.Tensor) and kontext_image.size(1) == self.latent_channels):
            img = kontext_image[0] if isinstance(kontext_image, list) else kontext_image
            ...
            kontext_image = self.image_processor.resize(kontext_image, image_height, image_width)
            kontext_image = self.image_processor.preprocess(kontext_image, image_height, image_width)
            # kontext_image = self.image_processor.resize(kontext_image, 2 * image_height, 2 * image_width)
            # kontext_image = self.image_processor.preprocess(kontext_image, 2 * image_height, 2 * image_width)
        ...
        if isinstance(self.controlnet, FluxControlNetModel):
            ...
            tem_width = width // 2
            tem_height = height // 2
            control_image, width, height = self.prepare_image_with_mask(
                image=control_image,
                mask=control_mask,
                width=width,
                height=height,
        ...
        # ! 无修改
        num_channels_latents = self.transformer.config.in_channels // 4
        latents, kontext_image_latents, latent_image_ids, kontext_image_ids = self.prepare_latents(
            kontext_image,
            batch_size * num_images_per_prompt,
            num_channels_latents,
            height,
        ...
        if kontext_image_latents is not None:
        ...
        ar_mask = torch.zeros(latents.shape[1], device=latents.device)
        ar_latent_input = latents 
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if kontext_image_latents is not None:
                    latent_model_input = torch.cat([latents, kontext_image_latents], dim=1)
                else: 
